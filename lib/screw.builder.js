var Screw = (function($) {
  ...
  screw.Describe = function(parent, name, element) {
    ...
    this.describe = function(name, fn) {
      try {
        var describe = push_to(this.children, new Screw.Describe(this, name));
        fn.call(describe);
        return describe;
      } finally {
        Screw.current_describe = this;
    ...
    this.it = function(name, fn) {
      return push_to(this.children, new Screw.It(this, name, fn));
    }
    ...
    if(!element) {
      ...
      this.parent.element.children('.describes').append(this.element);
    } else {
  ...
  screw.It = function(describe, name, fn) {
    ...
    this.element = $('<li class="it" name="'+ element_name + '">')
        .append($('<h2>').text(name));
    this.parent.element.children('.its').append(this.element);
    this.run = function() {
      try {
        try {
  ...
  $(function() {
    var top_level_describe_element = $('<div class="describe" name="Screw.describes[0]">')
      .append('<h3 class="status">')
      .append('<ul class="describes">')
      .appendTo('body');
    new Screw.Describe(null, "Top Level", top_level_describe_element);
    $(screw).dequeue();
